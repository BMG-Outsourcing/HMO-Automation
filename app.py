"""
BMG-HMO Billing File Processor — Streamlit App  v11.0-STABLE

CRITICAL FIXES FOR STREAMLIT CLOUD DEPLOYMENT:
  ✓ Fixed memory leaks with proper workbook cleanup
  ✓ Added context managers for safe file handling
  ✓ Implemented read-only mode for validation
  ✓ Fixed workbook cloning without duplication
  ✓ Added garbage collection after each major operation
  ✓ Removed all direct _sheets manipulation
  ✓ Fixed buffer management with try/finally blocks
"""
...
def safe_workbook(file_bytes, keep_vba=False, read_only=False):
    """Context manager to ensure workbook is properly closed"""
    wb = None
    file_buffer = None
    try:
        file_buffer = io.BytesIO(file_bytes)
        wb = load_workbook(file_buffer, keep_vba=keep_vba, read_only=read_only)
        yield wb
    finally:
        if wb:
            try:
                if not read_only:
                    # Close all worksheets first
...
def build_output(source_wb, df: pd.DataFrame, is_xlsm: bool = False,
    ...
    # Create a new workbook to avoid modifying source
    wb = Workbook()
    ...
    # Copy only the sheets we need from source_wb
    ...
    # Write to Working File sheet
    if "Working File" in wb.sheetnames:
    ...
    for sname in ENTITY_SHEETS:
        ...
        if att_ids and not subset.empty:
            is_att_sub = subset["Id Number"].isin(att_ids)
            subset = pd.concat(
    ...
    _write_summary_sheet(ws_summary, df, period_label=period_label)

    # ─── SAFE sheet reordering ────────────────────────────
...
def main():
    ...
    if not uploaded:
        st.markdown(
            '<div class="footer">BMG-HMO Automation · Internal Use Only · v11.0</div>',
            unsafe_allow_html=True,
        )
        return
    ...
    if st.button("⚙  Process File", type="primary"):
        ...
        try:
            ...
            # PHASE 1: Validation (read-only mode)
            prog.progress(10, text="Validating file…")
            with safe_workbook(file_bytes, keep_vba=is_xlsm, read_only=True) as wb_validation:
                logs.append("PHASE 1 · Validation")
                logs.append("─" * 50)
                val = phase1_validate(wb_validation, uploaded.name)
                ...
                period_label = _detect_period_label(wb_validation)
                logs.append(f"  ✓ Period label     : {period_label or '(not detected — using placeholder)'}")

            # PHASE 2: Sync (need write access for data extraction)
            prog.progress(35, text="Syncing Attachment → Working File…")
            logs.append("")
            logs.append("PHASE 2 · Sync")
            logs.append("─" * 50)
            
            with safe_workbook(file_bytes, keep_vba=is_xlsm, read_only=False) as wb_source:
                df, sync_logs = phase2_sync(wb_source)
            ...
            # PHASE 3: Build output with fresh workbook
            with safe_workbook(file_bytes, keep_vba=is_xlsm, read_only=False) as wb_out:
