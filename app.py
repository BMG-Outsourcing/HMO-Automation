...
# ─── CONTEXT MANAGER ──────────────────────────────────────────────────────────
@contextmanager
def safe_workbook(file_bytes, keep_vba=False, read_only=False):
    wb = buf = None
    try:
        buf = io.BytesIO(file_bytes)
        wb  = load_workbook(buf, keep_vba=keep_vba, read_only=read_only)
        yield wb
    finally:
        if wb:
            try:
...
def _concat_preserve(base: pd.DataFrame, *others) -> pd.DataFrame:
    ...
    result.attrs.update(saved)
    return result


# ─── ROW FILTER ───────────────────────────────────────────────────────────────
def _is_header_id(val) -> bool:
    return str(val).strip().lower() in _HEADER_ID_VALUES
...
def _log_id_info(raw_series: pd.Series, label: str, logs: list):
    ...
    if not header_like.empty:
        logs.append(f"  ℹ {label}: {len(header_like)} header-like ID value(s) skipped:")
    ...
    if not non_std.empty:
        logs.append(f"  ℹ {label}: {len(non_std)} non-standard ID(s) kept:")
...
def _read_sheet(wb, sheet_name: str) -> pd.DataFrame:
    ...
    cols, seen = [], {}
    for h in raw_hdr:
        key = _s(h) if h is not None else "_blank"
        if not key:
            key = "_blank"
        if key in seen:
            seen[key] += 1
...
def _detect_period_label(wb) -> str:
    ...
    # Fallback: any cell with 'month'
    month_pat = re.compile(r'month', re.IGNORECASE)
    for sname in wb.sheetnames:
        ws = wb[sname]
        for r in range(1, 25):
            for c in range(1, ws.max_column + 1):
                v = ws.cell(row=r, column=c).value
                if v and month_pat.search(str(v)):
                    return str(v).strip()
    return ""
...
def phase1_validate(wb, filename: str) -> dict:
    ...
    if not result["monthly"]:
        raise ValueError(
            "Validation failed: 'MONTHLY' keyword not found in filename or file contents."
        )

    if not result["has_attachment"] and not result["has_working_file"]:
        raise ValueError(
            "Neither 'Attachment' nor 'Working File' sheet found. "
...
def phase2_sync(wb) -> tuple[pd.DataFrame, list[str]]:
    ...
    has_wf  = "Working File" in wb.sheetnames
    ...
    if has_wf:
        ...
        if "Id Number" not in wf.columns:
            candidates = [c for c in wf.columns
                          if "id" in c.lower() and "number" in c.lower()]
            if candidates:
                wf = wf.rename(columns={candidates[0]: "Id Number"})
                logs.append(f"  ⚠ Renamed '{candidates[0]}' → 'Id Number'")
            else:
                raise KeyError("'Id Number' column not found in Working File.")
        ...
        if dup_mask.any():
            dup_ids = wf.loc[dup_mask, "Id Number"].unique()
            logs.append(f"  ⚠ Duplicate IDs in WF ({len(dup_ids)}) — ALL rows kept:")
    ...
    # ── If no Attachment, return WF as-is ─────────────────────────────────────
    if not has_att:
    ...
    logs.append(
        ...
        f"{list(att.columns)[:14]}{'…' if len(att.columns) > 14 else ''}"
    )

    if "Id Number" not in att.columns:
        candidates = [c for c in att.columns
                      if "id" in c.lower() and "number" in c.lower()]
        if candidates:
            att = att.rename(columns={candidates[0]: "Id Number"})
            logs.append(f"  ⚠ Renamed '{candidates[0]}' → 'Id Number' in Attachment")
        else:
            logs.append("  ✗ CRITICAL: 'Id Number' not found in Attachment — sync skipped!")
            wf.attrs["att_ids"] = set()
            return wf, logs
    ...
    # ── Append rows that are in Attachment but not in WF ──────────────────────
    new_count = 0
    for rid in sorted(att_ids - wf_ids):
        ...
        new_count += 1
        sc_display = sc if sc else "(blank — fill in Entity column)"
        logs.append(
            f"  + Added from Attachment       : {rid}  {_s(att_row.get('Name', ''))}  "
            f"[SC: {sc_display}]"
        )

    if new_count:
        logs.append(f"  Total new rows added          : {new_count}")
    ...
    blank_entity = wf["Entity"].apply(lambda v: _s(v) == "")
    if blank_entity.any():
        logs.append(
...
def _finalise_sc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _sc(row):
        ...
        if existing:
            return existing
        entity = _s(row.get("Entity", ""))
        return derive_system_coding(entity) if entity else ""
...
def _sort_by_sc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = (
        df.sort_values(["_sc_rank", "System Coding"], kind="stable")
          .drop(columns=["_sc_rank"])
...
def _write_summary_sheet(ws, df: pd.DataFrame, period_label: str = ""):
    ...
    def _group_sum(group_name: str, col: str, advance: bool | None = None) -> float:
        ...
        if advance is True:
            mask &= (df.get("System Coding",
                             pd.Series([""] * len(df), index=df.index))
                       .astype(str)
                       .str.contains("Advance", case=False, na=False))
...
# ─── SHARED TABLE WRITER ──────────────────────────────────────────────────────
def _write_table(ws, df_subset: pd.DataFrame, att_ids: set, header_row: int):
...
def _write_data_sheet(ws, df_subset: pd.DataFrame, att_ids: set):
    """Working File sheet — header at row 18."""
    _write_table(ws, df_subset, att_ids, header_row=HEADER_ROW)


def _write_data_sheet_entity(ws, df_subset: pd.DataFrame, att_ids: set):
    """Entity sheets (NYFD, BMG Internal) — header at row 1."""
    _write_table(ws, df_subset, att_ids, header_row=1)
    if not df_subset.empty:
        _write_grand_totals_row(ws, df_subset, 1 + 1 + len(df_subset))
...
# ─── SLEEK SHEET WRITER ───────────────────────────────────────────────────────
def _write_sleek_sheet(ws, df_sleek: pd.DataFrame, att_ids: set):
...
def _company_root(name: str) -> str:
    ...
    if not tokens:
        return name.lower()
    return tokens[0] if len(tokens) == 1 else " ".join(tokens[:2])


def _normalise_so_companies(names: list) -> dict:
...
# ─── SO SHEET WRITER ──────────────────────────────────────────────────────────
def _write_so_sheet(ws, df_so: pd.DataFrame, att_ids: set):
    n_cols      = len(WF_OUTPUT_COLS)
    last_letter = get_column_letter(n_cols)
...
def build_output(source_wb, df: pd.DataFrame, is_xlsm: bool = False,
    ...
    for sname in KEEP_SHEETS:
        if sname in source_wb.sheetnames:
            ...
            ws_source = source_wb[sname]
            for row in ws_source.iter_rows():
                for cell in row:
                    if cell.value is not None:
                        nc = ws_new.cell(row=cell.row, column=cell.column, value=cell.value)
                        if cell.has_style:
                            try:
    ...
    for sname in ENTITY_SHEETS:
        ...
        if att_ids and not subset.empty:
            is_att = subset["Id Number"].isin(att_ids)
            subset = pd.concat([subset[is_att], subset[~is_att]], ignore_index=True)
            subset.attrs.update(df.attrs)
        ...
        if sname == "Sleek":
            _write_sleek_sheet(ws_e, subset, att_ids)
        elif sname == "SO":
            _write_so_sheet(ws_e, subset, att_ids)
        else:
            _write_data_sheet_entity(ws_e, subset, att_ids)
...
def inject_css():
    ...
    <style>
        ...
        :root {
            ...
            --ash: #1C1C1C; --charcoal: #2E2E2E; --smoke: #F2EFEC;
            --bone: #FAF8F6; --mist: #EAE7E4; --steel: #6B6B6B;
            --silver: #A0A0A0; --line: #E0DDD9; --white: #FFFFFF;
            --green: #1A7A3C; --amber: #B45309;
        }
        ...
        .hero {
            ...
            background-image:
                ...
                repeating-linear-gradient(90deg, transparent, transparent 39px,
                    rgba(255,255,255,0.04) 39px, rgba(255,255,255,0.04) 40px);
            margin: -1rem -1rem 0 -1rem; padding: 0 40px;
            height: 88px; display: flex; align-items: center; gap: 20px;
        ...
        .hero-pill {
            ...
            font-size: 0.6rem; font-weight: 700; padding: 5px 12px;
        ...
        .sec-label-text {
            ...
            letter-spacing: 0.14em; text-transform: uppercase; color: var(--red-700);
            white-space: nowrap;
        }
        .steps-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 14px; margin: 28px 0 32px; }
        .step-card {
            background: var(--white); border: 1px solid var(--line);
            border-top: 3px solid var(--red-700); border-radius: 0 0 10px 10px;
            padding: 20px 18px 18px;
        }
        .step-num {
            font-family: 'Syne', sans-serif; font-size: 2rem; font-weight: 800;
            color: var(--red-100); line-height: 1; margin-bottom: 10px;
        }
        ...
        [data-testid="stFileUploader"] {
            border: 2px dashed var(--red-700) !important; border-radius: 10px !important;
            background: var(--red-50) !important; padding: 10px !important;
        }
        ...
        [data-testid="stBaseButton-primary"]:hover { background: var(--red-900) !important; }
        .result-banner {
            background: var(--white); border: 1px solid var(--line);
            border-left: 5px solid var(--red-700); border-radius: 8px;
            padding: 18px 22px; display: flex; align-items: center; gap: 16px; margin-bottom: 22px;
        }
        .result-icon {
            width: 40px; height: 40px; background: var(--red-700); border-radius: 8px;
            display: flex; align-items: center; justify-content: center; font-size: 1.1rem;
        }
        .result-text { font-family: 'Syne', sans-serif; font-size: 0.88rem; font-weight: 700; color: var(--ash); }
        ...
        .metric-card {
            background: var(--white); border: 1px solid var(--line);
            border-bottom: 3px solid var(--red-700); border-radius: 8px 8px 0 0; padding: 16px 18px 14px;
        }
        .metric-label {
            font-family: 'Syne', sans-serif; font-size: 0.58rem; font-weight: 700;
            text-transform: uppercase; letter-spacing: 0.12em; color: var(--red-700); margin-bottom: 8px;
        }
        .metric-value { font-family: 'Syne', sans-serif; font-size: 1.8rem; font-weight: 800; color: var(--ash); line-height: 1; }
        .metric-value.red   { color: var(--red-700); }
        .metric-value.green { color: var(--green); }
        .metric-value.amber { color: var(--amber); }
        [data-testid="stTabs"] [data-baseweb="tab"] {
            font-family: 'Syne', sans-serif !important; font-size: 0.74rem !important; font-weight: 700 !important;
        }
        [data-testid="stTabs"] [aria-selected="true"] { color: var(--red-700) !important; border-bottom-color: var(--red-700) !important; }
        .stCode, [data-testid="stCode"] {
            font-family: 'IBM Plex Mono', monospace !important; font-size: 0.71rem !important;
            background: var(--ash) !important; color: #E8E3DD !important; border-radius: 8px !important;
        }
        [data-testid="stExpander"] {
            border: 1px solid var(--line) !important; border-left: 3px solid var(--red-700) !important;
        ...
        [data-testid="stTextInput"] input:focus { border-color: var(--red-700) !important; }
        ...
        .warn-bar {
            ...
            padding: 10px 14px; font-size: 0.72rem; color: #92400E; margin-bottom: 14px;
        }
    </style>
...
def render_topbar():
    ...
    if os.path.exists(LOGO_PATH):
        ...
        with open(LOGO_PATH, "rb") as f:
            logo_html = (
                f'<img style="width:40px;height:40px;object-fit:contain;border-radius:6px;" '
    ...
    <div class="steps-grid">
        <div class="step-card">
            ...
            <div class="step-desc">
                ...
                It must contain the word <em>MONTHLY</em> and either an
                <em>Attachment</em> or <em>Working File</em> sheet (or both).
            </div>
        </div>
        <div class="step-card">
            ...
            <div class="step-desc">
                Financials are refreshed from the Attachment, new employees appended,
                and no existing row is removed. Manual / WF-only rows are flagged yellow.
                If no Working File exists it is created automatically.
            </div>
        </div>
        <div class="step-card">
            ...
            <div class="step-desc">
                Output contains <strong>Summary · Attachment · Working File ·
                Sleek · NYFD · BMG Internal · SO</strong>.
                Fill in missing Entity values for any yellow-highlighted rows.
            </div>
        </div>
    </div>
...
# ─── SYNC STATUS ──────────────────────────────────────────────────────────────
def render_sync_status(df: pd.DataFrame, att_ids: set):
    has_att = bool(att_ids)
    rows = []
    for _, row in df.iterrows():
        rid    = _s(row.get("Id Number", ""))
        entity = _s(row.get("Entity", ""))
        source = ("✅ From Attachment" if rid in att_ids else "⚠️ Manual / WF-only") \
    ...
    tabs = st.tabs([
        ...
        f"Sleek  ({(all_df['Sheet']=='Sleek').sum()})",
        f"NYFD  ({(all_df['Sheet']=='NYFD').sum()})",
        f"BMG Internal  ({(all_df['Sheet']=='BMG Internal').sum()})",
        f"SO  ({(all_df['Sheet']=='SO').sum()})",
    ])
    tab_all, tab_synced, tab_manual, tab_sleek, tab_nyfd, tab_bmg, tab_so = tabs

    with tab_all:
        st.caption("Every employee. **Sheet** column shows which entity tab they land in.")
        st.dataframe(all_df, use_container_width=True, hide_index=True)

    with tab_synced:
        if synced_df.empty:
            st.info("No employees matched the Attachment sheet.")
        else:
    ...
    for tab, sname in zip([tab_sleek, tab_nyfd, tab_bmg, tab_so], ENTITY_SHEETS):
        with tab:
            ...
            if sub.empty:
                ...
                st.caption(f"**{len(sub)}** row(s) → **Sleek** sheet.")
                for grp_label in SLEEK_SUBGROUPS:
                    grp_df = sub[sub["Sleek Group"] == grp_label].drop(
                        columns=["Sleek Group"]).reset_index(drop=True)
                    if grp_df.empty:
                        continue
                ...
                st.caption(f"**{len(display)}** row(s) → **SO** sheet.")
                ...
                for company in sorted(display["_company"].unique(), key=lambda x: x.lower()):
                    ...
                    if "System Coding" in co_df.columns:
                        ...
                        co_df = co_df.sort_values(["_sc_rank", "System Coding"], kind="stable").drop(
                            columns=["_sc_rank"]).reset_index(drop=True)
...
def render_add_employee(df: pd.DataFrame) -> "pd.DataFrame | None":
    ...
    with st.form("add_emp", clear_on_submit=True):
        ...
        sc = st.selectbox(
            "System Coding", [SC_AUTO_LABEL] + list(SC_TO_CAT.keys()), index=0,
...
def main():
    ...
    st.markdown('<div class="upload-label">Upload Billing File (.xlsx / .xlsm)</div>',
                unsafe_allow_html=True)
    uploaded = st.file_uploader("Billing File Upload", type=["xlsx", "xlsm"],
    ...
    for key, default in [
        ("df", None), ("out_bytes", None), ("file_bytes", None),
        ("is_xlsm", False), ("out_name", ""), ("sync_summary", {}),
    ...
    if st.button("⚙  Process File", type="primary"):
        ...
        try:
            ...
            with safe_workbook(file_bytes, keep_vba=is_xlsm, read_only=True) as wb_v:
                logs.append("PHASE 1 · Validation")
                logs.append("─" * 50)
                val = phase1_validate(wb_v, uploaded.name)
            ...
            logs.append("─" * 50)
            with safe_workbook(file_bytes, keep_vba=is_xlsm, read_only=False) as wb_s:
                df, sync_logs = phase2_sync(wb_s)
            ...
            with safe_workbook(file_bytes, keep_vba=is_xlsm, read_only=False) as wb_o:
                wb_o      = _scrub_workbook(wb_o)
                out_bytes = build_output(wb_o, df, is_xlsm=is_xlsm,
    ...
    if st.session_state.df is not None:
        ...
        if no_entity:
            st.markdown(
                f'<div class="warn-bar">⚠️ <strong>{no_entity} row(s)</strong> have no '
                f'<em>Entity</em> set — they land in the <strong>SO</strong> sheet by default. '
                f'Open the downloaded file and fill in their Entity column, then re-upload.</div>',
                unsafe_allow_html=True,
            )
        ...
        <div class="result-banner">
            ...
            <div>
                ...
                <div class="result-sub">{ss.get('total', 0)} records · 7 sheets · Summary included</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.expander("🔍  View processing log", expanded=False):
            st.code("\n".join(st.session_state.proc_logs), language=None)
        ...
        if search:
            ...
            found = df[mask]
            if not found.empty:
                fd = found[[c for c in WF_OUTPUT_COLS if c in found.columns]].copy()
                fd["Sheet"]  = found["Entity"].apply(lambda e: _entity_sheet(_s(e)))
                fd["Source"] = found["Id Number"].apply(
        ...
        if st.checkbox("➕  Add a new employee to the Working File", value=False,
            ...
            if updated_df is not None:
                with safe_workbook(st.session_state.file_bytes,
                                   keep_vba=st.session_state.is_xlsm,
                                   read_only=False) as wb2:
